...
def get_index_time(index_timestamp, separator='-'):
    ...
    """
    try:
        return datetime.strptime(index_timestamp, separator.join(('%d', '%m', '%Y', '%H')))
    except ValueError:
        return datetime.strptime(index_timestamp, separator.join(('%d', '%m', '%Y')))
...
def get_snaplist(client, repo_name, prefix=''):
...
def get_snapped_indices(client, repo_name, prefix=''):
...
def find_overusage_indices(client, disk_space=2097152.0, separator='-', prefix='', **kwargs):
